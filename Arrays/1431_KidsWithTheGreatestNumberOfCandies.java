...
class Solution {
    public List<Boolean> kidsWithCandies(int[] candies, int extraCandies) {
        ...
        int maxC = Arrays.stream(candies).max().getAsInt(); // Find the maximum number of candies
        List<Boolean> result = new ArrayList<>();

        for (int i = 0; i < n; i++) {
            // If the current child can have the max number of candies with the extra
            // candies
            if (candies[i] + extraCandies >= maxC) {
                result.add(true); // Add true if they can have the most candies
            } else {
                result.add(false); // Add false otherwise
            }
        }
