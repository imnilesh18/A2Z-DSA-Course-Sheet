...
class Solution {
    ...
    static int findPlatform(int arr[], int dep[], int n) {
        ...
        int cnt = 0; // Current count of platforms needed
        int maxCnt = 0; // Maximum count of platforms needed

        while (i < n && j < n) {
            // If a train is arriving before the earliest train departs
            if (arr[i] <= dep[j]) {
                cnt++; // Need a new platform
                i++;
            } else {
                cnt--; // One train departs, release a platform
                j++;
            }
