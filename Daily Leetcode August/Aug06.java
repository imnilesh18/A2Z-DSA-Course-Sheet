...
public class Solution {
    ...
    public int minimumPushes(String word) {
        // Initialize an ArrayList to store counts
...
public class Solution {
    ...
    public int minimumPushes(String word) {
        ...
        // Sort the counts in descending order
        // Convert int array to Integer array for sorting with comparator
        Integer[] countsWrapper = Arrays.stream(counts).boxed().toArray(Integer[]::new);

        // Sort the counts array in descending order using a comparator
        Arrays.sort(countsWrapper, Collections.reverseOrder());

        int res = 0;
        int distinct = 0;

        // Calculate the minimum pushes
        for (int count : countsWrapper) {
            if (count > 0) {
                res += count * (1 + distinct / 8);
                distinct++;
            }
